...
- タグによる整理と絞り込みができる
- 管理者専用の評価機能として、各画像にハートとスターをそれぞれ 1〜5 段階で付けられる
...
- Cloud Firestore: 画像メタデータ、タグ、管理者評価管理
...
`sortAt` はギャラリー上の表示順に使う日時です。初期値はアップロード時点の日時とし、管理者が必要に応じて後から編集できます。通常は `sortAt` の降順で一覧表示します。

`takenAt` は実際の撮影日時や写真の由来に関する記録用として保持し、必要に応じて `sortAt` と同じ値にも別の値にもできます。

`heartRating` と `starRating` は管理者専用の評価項目です。未評価状態を扱えるように `null` を許容し、評価が設定されている場合は 1〜5 の整数値を持ちます。

## 並び順の設計

ギャラリーの表示順は `sortAt` を基準に決定します。

- 初期状態では `sortAt = createdAt` とする
- 通常のアップロードではアップロード順に並ぶ
- 後から受け取った写真や、時系列を調整したい画像は管理者が `sortAt` を編集する
- 一覧表示は `sortAt` の降順を基本とする
- 必要に応じて `takenAt` は別途保持し、表示用日時と記録用日時を分ける

この設計により、公開ギャラリーは自然な時系列を保ちつつ、管理者が後から柔軟に順序調整できます。
...
- 画像投稿が可能
- メタデータ編集と公開状態変更が可能
- `sortAt` の編集により表示順を調整可能
- ハート評価とスター評価を個別に設定可能
- 管理画面へアクセス可能
- 署名付き URL を通じて高解像度画像を閲覧可能
- 画像詳細画面で評価 UI を直接操作可能
...
- 管理画面では日時編集によりギャラリー表示順を調整できる
- 管理者ログイン時は画像詳細画面にハートとスターの評価 UI を表示する
- 評価 UI は画像を見ながらすぐ更新できる配置にする
...
- ハート評価とスター評価は管理者専用であり、公開向けのソーシャル機能ではない
